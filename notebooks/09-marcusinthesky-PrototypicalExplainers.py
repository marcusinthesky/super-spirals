...
for i, (proto, label) in enumerate(zip(prototypes, prototype_labels)):
    grid = proto.numpy().reshape((8, 8))
    standard_grid = (grid - grid.min()) / grid.max()

    images.append(
        hv.Image(standard_grid).opts(
            title=f"Digit: {str(label)} ------------------------------ Leaf : {i+1}",
            xlabel="",
            ylabel="",
            xaxis=None,
            yaxis=None,
...
boston_X_z_score = tf.math.divide_no_nan(
    boston_X - tf.reduce_mean(boston_X, 0), tf.math.reduce_std(boston_X, 0)
)
...
#%%
boston_tree = SoftDecisionTreeRegressor(max_depth=1, classes=1, beta=0.1, alpha=1.0)
...
def weight_plots(index: int, leaf: tf.keras.layers.Layer) -> hv.plotting.Plot:
    return pipe(
        zip(boston.feature_names, leaf.w.numpy().reshape((-1,)).tolist()), hv.Bars
    ).opts(
        xrotation=90,
        xlabel="Features",
        ylabel="Weights",
...
#%%
pipe(
    enumerate(boston_tree.tree[-1]), map(lambda leaf: weight_plots(*leaf)), reduce(add)
)

#%%
pipe(
    boston_X_z_score,  # the data
    ...
    hv.Bars,  # gen polts
).opts(title="% sample per leaf", xlabel="Leaves", ylabel="%")
    ...
    return tf.keras.Sequential(
        [
            ...
            tfp.layers.DistributionLambda(
                lambda t: tf.distributions.Independent(
...
def prior(kernel_size, bias_size=0, dtype=None):
    ...
    return tf.keras.Sequential(
        [
            ...
            tfp.layers.DistributionLambda(
                lambda t: tf.distributions.Independent(
                    tf.distributions.Normal(loc=t, scale=1), reinterpreted_batch_ndims=1
                )
            ),
        ]
    )
...
model = tf.keras.Sequential(
    [
        tfp.layers.DenseVariational(
            ...
            prior,
            kl_weight=1 / boston_X_z_score.shape[0],
        ),
...
rv_mix_probs = tfp.distributions.Dirichlet(
    concentration=np.ones(components, dtype) / 10.0, name="rv_mix_probs"
)
...
rv_precision = tfp.distributions.Gamma(
    concentration=[1.0] * components, rate=[2.0] * components
)
...
def joint_log_prob(mix_probs, beta, sigma):
    ...
    rv_observations = tfp.distributions.MixtureSameFamily(
        ...
        components_distribution=tfp.distributions.Normal(loc=X @ beta, scale=sigma),
    )
...
unnormalized_posterior_log_prob = (
    joint_log_prob  # functools.partial(joint_log_prob, X=X, y=y.reshape(-1,1))
)

initial_state = [
    tf.fill([components], value=np.array(1.0 / components, dtype), name="mix_probs"),
    tf.constant(tf.random.normal(shape=(dims, components), dtype=dtype), name="loc"),
...
plots = [
    hv.Bars(
        zip(data.feature_names, betas[:, l]), label=f"Feature importance at mixture {l}"
    ).opts(xrotation=45)
    for l in range(2)
]
reduce(add, plots)
