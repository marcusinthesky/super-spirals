...
def test__p_conditional():
    ...
    tf.debugging.assert_near(tf.reduce_sum(p, axis=1), 
                             tf.ones((tf.shape(p)[0],)), 0)

@tf.function(experimental_relax_shapes=True)
def p_perplexity(d_true, sigma):
    p_true_conditional = p_conditional(d_true, sigma)
    
    entropy = -1 *tf.math.divide_no_nan(p_true_conditional * (tf.math.log(p_true_conditional)), tf.math.log(2.0))
...
def find_optimal_sigmas(d_true, preplexity):
    ...
    sigmas = binary_search(lambda sigma: p_perplexity(d_true, sigma), 
                           preplexity)
    return sigmas


@tf.function(experimental_relax_shapes=True)
def tsne_loss(y_true, y_pred, p_true=2, p_pred=2, perpexity = 1.):
    length = tf.shape(y_true)[0]
    
    null_diag = tf.dtypes.cast(tf.linalg.set_diag(tf.ones((length,length)), 
                                                     tf.zeros((length))),
                               y_true.dtype)
    ...
    return tf.reduce_mean(p_true_null * tf.math.log(tf.math.divide_no_nan(p_true_null, q_pred_null)))
