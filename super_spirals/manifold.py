...
class _ParametricManifold(tf.keras.Model, BaseEstimator, TransformerMixin):
    def __init__(
        ...
        *hidden_units, final_unit = hidden_layer_sizes
        ...
        self.final_transformation = tf.keras.layers.Dense(
            final_unit, activation="linear"
        )
    ...
    def fit(self, X: Union[np.ndarray, tf.Tensor], y=None):
        self.compile(optimizer=self.solver_, loss=self.manifold_loss)
...
class ParametricTSNE(_ParametricManifold):
    def __init__(
        ...
        norm_order_input: float = 1.0,
        norm_order_latent: float = 2.0,
        hidden_layer_sizes: Tuple[int] = (4, 2),
        alpha: float = 0.01,
        l1_ratio: float = 0.0,
        activation: str = "selu",
        ...
        super(ParametricTSNE, self).__init__(
            manifold_loss=lambda y_true, y_pred: tsne_loss(
                y_true,
                y_pred,
...
class ParametricMDS(_ParametricManifold):
    def __init__(
        ...
        super(ParametricMDS, self).__init__(
            manifold_loss=lambda y_true, y_pred: stress_loss(
                y_true, y_pred, p_true=norm_order_input, p_pred=norm_order_latent
            ),
            hidden_layer_sizes=hidden_layer_sizes,
            alpha=alpha,
            l1_ratio=l1_ratio,
...
class ParametricSammonMapping(_ParametricManifold):
    def __init__(
        ...
        super(ParametricSammonMapping, self).__init__(
            manifold_loss=lambda y_true, y_pred: strain_loss(
                y_true, y_pred, p_true=norm_order_input, p_pred=norm_order_latent
            ),
            hidden_layer_sizes=hidden_layer_sizes,
            alpha=alpha,
...
class ParametricLLE(_ParametricManifold):
    def __init__(
        ...
        super(ParametricLLE, self).__init__(
            manifold_loss=lambda y_true, y_pred: lle_loss(
                y_true, y_pred, minkowski=norm_order, neighours=neighours
            ),
            hidden_layer_sizes=hidden_layer_sizes,
            alpha=alpha,
            l1_ratio=l1_ratio,
