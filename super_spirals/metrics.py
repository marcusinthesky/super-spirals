...
def get_latent_space(model, 
    ...
    return (pd.concat([latent], axis=1)
            .assign(label=y_test)
            .assign(label = lambda d: d.label.replace(labels))
            .assign(tag = tag))

def reconstruction_benchmark(dataset: Dict[str,np.ndarray], 
                             models: Dict[str,TransformerMixin],
                             label:str):
    """
    """
    data = dataset
    ...
    latent = pd.concat([get_latent_space(m, 
                                         X_train, 
                                         X_test, 
                                         y_train, 
                                         y_test, 
    ...
    reconstruction_loss = pd.DataFrame({t: pipe(X_train,
                                                 m.transform,
                                                 m.inverse_transform,
                                                 lambda x: np.subtract(x, X_train),
                                                 lambda x: np.power(x,2),
                                                 lambda x: np.array(x).flatten(),
                                                 np.mean) for t, m in models.items()},
                                       index=['Reconstruction Loss']).T

    silhouette = pd.DataFrame({t: [silhouette_score(X=pipe(X_test, m.transform),
                                                    labels=y_test)]
                               for t,m in models.items()}, index=['Silhoutte']).T
    

    return (latent, 
            reconstruction_loss.hvplot.bar(title=f'{label}: Reconstruction Loss'), 
            silhouette.hvplot.bar(title=f'{label}: Silhouette Scores'), 
            latent.hvplot.scatter(x='Component 1', 
                                  y='Component 2', 
                                  color='label',
                                  groupby='tag',
                                  label=label).layout().cols(2))