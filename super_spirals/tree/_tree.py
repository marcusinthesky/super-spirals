...
class Node(tf.keras.layers.Layer):
    ...
    def call(self, inputs, probability):
        ...
        return (
            (pr * inputs, pr_one * probability),
            ((1 - pr) * inputs, (1 - pr_one) * probability),
        )
...
class Leaf(tf.keras.layers.Layer):
    ...
    def call(self, inputs: tf.Tensor, probability):
        ...
        l1 = tf.reduce_sum(0.0 * tf.abs(self.w))
...
class ClassificationHead(tf.keras.layers.Layer):
    ...
    def call(self, inputs: tf.Tensor):
        ...
        weighted_inputs = [
            tf.nn.softmax(x[0]) * tf.reshape(x[1], (-1, 1)) for x in inputs
        ]
...
class SoftDecisionTree(tf.keras.Model, BaseEstimator, RegressorMixin):
    def __init__(
        ...
        self.nodes = [
            [Node(1, beta=beta, l1=l1) for _ in range(2 ** layer)]
    ...
    def fit(self, X: Union[np.ndarray, tf.Tensor], y=None):
        ...
        self.compile(optimizer=self.solver_, loss=self.loss)
...
class SoftDecisionTreeRegressor(SoftDecisionTree):
    def __init__(
        ...
        super(SoftDecisionTreeRegressor, self).__init__(
            max_depth=max_depth,
            classes=classes,
            beta=beta,
            alpha=alpha,
