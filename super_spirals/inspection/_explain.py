...
@tf.function
def explainer_mse_loss(
    y_true: tf.Tensor, y_pred: tf.Tensor, sample_weights: tf.Tensor = None
):
...
class _TabularExplainer(tf.keras.Model, BaseEstimator):
    ...
    def train_loop(self, batches: tf.data.Dataset):
        for epoch in range(self.n_iter):
            for sample in batches:
                ...
                explainer_train_step(
                    index_sample, X_sample, y_sample, sample_weights, self, self.solver_
                )
    ...
    def fit_transform(self, X: np.ndarray, y: np.ndarray):
        ...
        self.feature_importances = tf.keras.layers.Embedding(
            self.X_shape[0] + 1,
            self.X_shape[1],
            embeddings_initializer="normal",
            embeddings_regularizer=tf.keras.regularizers.l1_l2(
                l1=self.alpha * self.l1_ratio, l2=self.alpha * (1 - self.l1_ratio)
            ),
        )
        ...
        batches = tf.data.Dataset.from_tensor_slices(
            (index, X.astype("float32"), y.astype("float32"))
...
class LimeTabularExplainer(_TabularExplainer):
    def __init__(
        ...
        kernel = tfp.math.psd_kernels.ExponentiatedQuadratic(1.0, kernel_width)
    ...
    def train_loop(self, batches: tf.data.Dataset):
        for epoch in range(self.n_iter):
            for sample in batches:
                ...
                z_prime = tf.dtypes.cast(
                    tf.random.uniform(tf.shape(X_sample)) > 0.5, "float32"
                )
                ...
                explainer_train_step(
                    index_sample, z, y_pred, sample_weights, self, self.solver_
                )
